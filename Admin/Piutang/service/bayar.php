...
$piutang = $conn->query("SELECT pi.*, p.no_invoice, c.nama_customer 
                         FROM piutang_service pi
                         JOIN penjualan_service p ON pi.penjualan_id=p.id
...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    $conn->query("INSERT INTO piutang_service_pembayaran (piutang_id,tanggal,jumlah) 
                  VALUES ($id,'$tanggal',$jumlah)");
    $sisa = $piutang['sisa_piutang'] - $jumlah;
    $status = $sisa<=0?'Lunas':'Belum Lunas';
    $conn->query("UPDATE piutang_service SET sisa_piutang=$sisa, status='$status' WHERE id=$id");
...
<div class="container">
  <h4>Pembayaran Piutang Service</h4>
