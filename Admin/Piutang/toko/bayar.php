...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    $conn->query("INSERT INTO piutang_toko_pembayaran (piutang_id,tanggal,jumlah) 
                  VALUES ($id,'$tanggal',$jumlah)");
    $sisa = $piutang['sisa_piutang'] - $jumlah;
    $status = $sisa<=0?'Lunas':'Belum Lunas';
    $conn->query("UPDATE piutang_toko SET sisa_piutang=$sisa, status='$status' WHERE id=$id");
